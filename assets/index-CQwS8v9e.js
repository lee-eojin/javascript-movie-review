(function(){const e=document.createElement("link").relList;if(e&&e.supports&&e.supports("modulepreload"))return;for(const a of document.querySelectorAll('link[rel="modulepreload"]'))n(a);new MutationObserver(a=>{for(const o of a)if(o.type==="childList")for(const c of o.addedNodes)c.tagName==="LINK"&&c.rel==="modulepreload"&&n(c)}).observe(document,{childList:!0,subtree:!0});function s(a){const o={};return a.integrity&&(o.integrity=a.integrity),a.referrerPolicy&&(o.referrerPolicy=a.referrerPolicy),a.crossOrigin==="use-credentials"?o.credentials="include":a.crossOrigin==="anonymous"?o.credentials="omit":o.credentials="same-origin",o}function n(a){if(a.ep)return;a.ep=!0;const o=s(a);fetch(a.href,o)}})();class f{listeners=new Map;subscribe(e,s){this.listeners.has(e)||this.listeners.set(e,[]),this.listeners.get(e).push(s)}publish(e,s){this.listeners.get(e)?.forEach(n=>n(s))}}const r=new f,i={TITLE_CHANGED:"titleChanged",LOGO_CLICK:"logoClick",LOAD_START:"loadStart",LOAD_MORE:"loadMore",LAST_PAGE_REACHED:"lastPageReached",MOVIES_LOADED:"moviesLoaded",SEARCH_LOADED:"searchLoaded",MORE_LOADED:"moreLoaded",MOVIE_SELECTED:"movieSelected",MOVIE_DETAIL_LOADED:"movieDetailLoaded",MODAL_CLOSED:"modalClosed",ERROR:"error"},O="data:image/png;base64,",D="data:image/png;base64,",y="data:image/png;base64,",T="https://image.tmdb.org/t/p/w1920_and_h800_multi_faces/",U="https://media.themoviedb.org/t/p/w200",P="https://image.tmdb.org/t/p/w500",B=document.querySelector(".main-title"),Y=document.querySelector(".scroll-sentinel"),h=document.querySelector(".background-container");function E(){const t=document.querySelector(".logo");t&&t.addEventListener("click",()=>{r.publish(i.LOGO_CLICK,void 0)},{once:!0})}const u={clearHeader(){const t=h;t.innerHTML="",t.style.background="none",t.style.height="100px"},render(t){if(!t)return;const e=h;e.innerHTML=`
            <div class="top-rated-movie">
                <div class="rate">
                  <img src="${O}" class="star" />
                  <span class="rate-value">${(t.vote_average??0).toFixed(1)}</span>
                </div>
                <div class="title">${t.title}</div>
                <button class="primary detail">자세히 보기</button>
            </div>
            ${this.renderImage()}
      `,e.style.background=`url(${T}${t.backdrop_path}) no-repeat center center / cover`,e.style.removeProperty("height"),E(),e.querySelector(".detail")?.addEventListener("click",()=>{r.publish(i.MOVIE_SELECTED,t.id)})},renderSearch(){const t=h;t.innerHTML=`${this.renderImage()}`,E()},renderImage(){return`<div class="overlay" aria-hidden="true">
                <div class="search-container">
                <h1 class="logo">
                    <img src="${D}" alt="MovieList" />
                </h1>
                <form class="search-form">
                    <input
                    type="search"
                    class="search-input"
                    placeholder="검색어를 입력하세요"
                    />
                    <button type="submit" class="btn-submit">
                        <img
                            src="${y}"
                            alt="search"
                            class="img-search"
                        />
                     </button>
                 </form>
                </div>
            </div>`}};class x{movie;constructor(e){this.movie=e}render(){return`<div class="item" data-id="${this.movie.id}">
      <img
         class="thumbnail"
         src="${U}${this.movie.poster_path}"
         alt="${this.movie.title}"
        />
        <div class="item-desc">
          <p class="rate">
            <img src="${O}" class="star" />
            <span>${(this.movie.vote_average??0).toFixed(1)}</span>
          </p>
          <strong>${this.movie.title}</strong>
        </div>
      </div>
    `}}class N{render(){return`<div class="movie-skeleton">
      <div class="movie-skeleton__poster"></div>
      <div class="movie-skeleton__details">
       <div class="movie-skeleton__title"></div>
       <div class="movie-skeleton__info"></div>
      </div>
    </div>
    `}}const H="/javascript-movie-review/assets/Nosearch-9C2OvmM-.png";class V{movieList;movieContainer;constructor(){this.movieList=document.querySelector(".thumbnail-list"),this.movieContainer=document.querySelector(".main-result"),this.attachClickListener()}attachClickListener(){this.movieList?.addEventListener("click",e=>{const s=e.target.closest("[data-id]");s&&r.publish(i.MOVIE_SELECTED,Number(s.dataset.id))})}showEmpty(){this.movieContainer.innerHTML=`
      <div class="result-none">
        <img src="${H}" alt="검색 결과 없음" class="result-none-image" />
        <p class="result-none-text">검색 결과가 없습니다.</p>
      </div>
    `}showError(e){this.movieContainer.innerHTML=`
      <div class="result-none">
        <p class="result-none-text">${e}</p>
      </div>
    `}clearList(){this.movieList.innerHTML="",this.movieContainer.innerHTML=""}renderSkeleton(){this.clearList();const e=new N().render();for(let s=0;s<20;s++){const n=document.createElement("li");n.innerHTML=e,this.movieList?.append(n)}}renderMovieList(e){e.results.forEach(s=>{const n=document.createElement("li"),a=new x(s);n.innerHTML=a.render(),this.movieList?.append(n)})}}const l=new V;class Q{constructor(e){this.sentinel=e,this.observer=new IntersectionObserver(this.onIntersect.bind(this))}observer;onIntersect(e){e[0].isIntersecting&&r.publish(i.LOAD_MORE,void 0)}observe(){this.observer.observe(this.sentinel)}disconnect(){this.observer.disconnect()}}const v="data:image/png;base64,",G="data:image/png;base64,",j="data:image/png;base64,",m=5,X=2,L={2:"최악이에요",4:"별로예요",6:"보통이에요",8:"재미있어요",10:"명작이에요"};class F{constructor(e,s,n){this.container=e,this.movieId=s,this.repository=n}currentRating=0;async mount(){const e=await this.repository.load(this.movieId);this.currentRating=e??0,this.container.innerHTML=this.buildHTML(),this.attach()}buildHTML(){const e=Array.from({length:m},(a,o)=>{const c=(m-o)*X,w=c===this.currentRating?"checked":"";return`
        <input type="radio" name="rating-${this.movieId}" id="star-${this.movieId}-${c}" value="${c}" ${w} />
        <label for="star-${this.movieId}-${c}" data-value="${c}" aria-label="${c}점"
          style="background-image: url(${j})"></label>
      `}).join(""),s=this.currentRating>0?L[this.currentRating]:"평가하기",n=this.currentRating>0?`<span class="rating-score">(${this.currentRating}/10)</span>`:"";return`<div class="star-list">${e}</div><span class="rating-label">${s} ${n}</span>`}updateLabel(e,s){const n=s>0?L[s]:"평가하기",a=s>0?`<span class="rating-score">(${s}/10)</span>`:"";e.innerHTML=`${n} ${a}`}attach(){const e=this.container.querySelector(".star-list"),s=this.container.querySelector(".rating-label");e.addEventListener("change",async n=>{const a=n.target;this.currentRating=Number(a.value),await this.repository.save(this.movieId,this.currentRating),this.updateLabel(s,this.currentRating)})}}const b="rating_";class k{async save(e,s){localStorage.setItem(`${b}${e}`,String(s))}async load(e){const s=localStorage.getItem(`${b}${e}`);return s!==null?Number(s):null}}class J{constructor(e){this.ratingRepository=e,this.dialog=document.querySelector(".modal"),this.dialog.addEventListener("close",()=>{r.publish(i.MODAL_CLOSED,void 0)})}dialog;openWithLoading(){this.dialog.innerHTML=this.renderLoading(),this.dialog.showModal()}fill(e){this.dialog.innerHTML=this.render(e);const s=this.dialog.querySelector(".close-modal");s.focus(),s.addEventListener("click",()=>this.dialog.close());const n=this.dialog.querySelector(".star-rating");new F(n,e.id,this.ratingRepository).mount()}renderLoading(){return`
      <button class="close-modal">
        <img src="${v}" alt="닫기" />
      </button>
      <div class="modal-loading">
        <div class="spinner"></div>
      </div>
    `}render(e){const s=e.release_date?e.release_date.slice(0,4):"미정",n=e.genres.length>0?e.genres.map(o=>o.name).join(", "):"장르 정보 없음",a=e.poster_path?`${P}${e.poster_path}`:"";return`
      <button class="close-modal">
        <img src="${v}" alt="닫기" />
      </button>
      <div class="modal-container">
        <div class="modal-image">
          <img src="${a}" alt="${e.title}" />
        </div>
        <div class="modal-description">
          <h2>${e.title}</h2>
          <p class="subtitle">${s} · ${n}</p>
          <div class="rate">
            <span>평균</span>
            <span class="rate-score">
              <img src="${G}" alt="별점" />
              <span>${(e.vote_average??0).toFixed(1)}</span>
            </span>
          </div>
          <hr class="modal-divider" />
          <p class="section-label">내 별점</p>
          <div class="star-rating"></div>
          <hr class="modal-divider" />
          <p class="section-label">줄거리</p>
          <p class="detail">${e.overview??"줄거리 정보가 없습니다."}</p>
        </div>
      </div>
    `}}const R=new J(new k);function Z(){const t=new Q(Y);r.subscribe(i.TITLE_CHANGED,e=>{B.textContent=e}),r.subscribe(i.LOAD_START,()=>{t.disconnect(),l.renderSkeleton()}),r.subscribe(i.MOVIES_LOADED,e=>{u.clearHeader(),u.render(e.results[0]??null),l.clearList(),l.renderMovieList(e),t.observe()}),r.subscribe(i.SEARCH_LOADED,e=>{u.clearHeader(),u.renderSearch(),e.results.length===0?(l.clearList(),l.showEmpty()):(l.clearList(),l.renderMovieList(e),t.observe())}),r.subscribe(i.MORE_LOADED,e=>{l.renderMovieList(e),t.observe()}),r.subscribe(i.LAST_PAGE_REACHED,()=>{t.disconnect()}),r.subscribe(i.MOVIE_SELECTED,()=>{t.disconnect(),R.openWithLoading()}),r.subscribe(i.MOVIE_DETAIL_LOADED,e=>{R.fill(e)}),r.subscribe(i.MODAL_CLOSED,()=>{t.observe()}),r.subscribe(i.ERROR,e=>{t.disconnect(),l.clearList(),l.showError(e)})}const M="https://api.themoviedb.org/3",C="53582fed17d630068894f05310efec62",z="movie/popular",q="search/movie",W="movie";class d extends Error{constructor(e,s){super(s),this.status=e,this.name="ApiError"}}class K extends d{constructor(){super(401,"인증에 실패했습니다."),this.name="UnauthorizedError"}}class _ extends d{constructor(){super(404,"요청한 리소스를 찾을 수 없습니다."),this.name="NotFoundError"}}class $ extends d{constructor(){super(429,"요청이 너무 많습니다. 잠시 후 새로고침 해주세요."),this.name="TooManyRequestsError"}}class ee extends d{constructor(){super(503,"서버가 일시적으로 사용 불가 상태입니다. 잠시 후 다시 시도해주세요."),this.name="ServiceUnavailableError"}}async function I(t){const e=await fetch(t);if(!e.ok)throw e.status===401?new K:e.status===404?new _:e.status===429?new $:e.status===503?new ee:new d(e.status,`API 요청 실패: ${e.status}`);return e.json()}async function S(t,e,s=""){const n=new URLSearchParams({api_key:C,language:"ko-KR",page:String(e),...s&&{query:s}});return I(`${M}/${t}?${n}`)}async function te(t){const e=new URLSearchParams({api_key:C,language:"ko-KR"});return I(`${M}/${W}/${t}?${e}`)}async function g(t,e){return e?await S(q,t,e):await S(z,t)}async function se(t){return await g(t)}async function re(t,e){return await g(t,e)}async function ie(t,e){return await g(t,e)}async function ne(t){return await te(t)}const A={page:1,searchQuery:""};async function ae(){await p(A.page)}async function oe(){A.page=1,A.searchQuery="",await p(A.page)}async function p(t){r.publish(i.LOAD_START,void 0);try{const e=await se(t);r.publish(i.MOVIES_LOADED,e)}catch(e){r.publish(i.ERROR,e.message)}}async function Ae(t){if(A.page=1,A.searchQuery=t,t===""){await p(A.page);return}await ce(A.page,t)}async function ce(t,e){r.publish(i.LOAD_START,void 0);try{const s=await re(t,e);r.publish(i.SEARCH_LOADED,s)}catch(s){r.publish(i.ERROR,s.message)}}async function le(){A.page+=1,await de(A.page,A.searchQuery)}async function de(t,e){try{const s=await ie(t,e);r.publish(i.MORE_LOADED,s),t>=s.total_pages&&r.publish(i.LAST_PAGE_REACHED,void 0)}catch(s){r.publish(i.ERROR,s.message)}}async function ue(t){try{const e=await ne(t);r.publish(i.MOVIE_DETAIL_LOADED,e)}catch(e){r.publish(i.ERROR,e.message)}}function he(){r.subscribe(i.LOAD_MORE,le),r.subscribe(i.MOVIE_SELECTED,ue),r.subscribe(i.LOGO_CLICK,()=>{r.publish(i.TITLE_CHANGED,"지금 인기 있는 영화"),oe()})}function ge(){addEventListener("load",async()=>{r.publish(i.TITLE_CHANGED,"지금 인기 있는 영화"),await ae()}),h.addEventListener("submit",async t=>{t.preventDefault();const s=document.querySelector(".search-input").value.trim(),n=s===""?"지금 인기 있는 영화":`"${s}" 검색 결과`;r.publish(i.TITLE_CHANGED,n),await Ae(s)})}Z();he();ge();
